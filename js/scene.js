...
(function () {
  ...
  function zoomToScreen() {
    ...
    mode = "toScreen";
    if (window.__fadeAmbience) window.__fadeAmbience(); // duck room ambience when entering the monitor
  ...
  function zoomHome() {
    ...
    backBtn.hidden = true;
    mode = "toHome";
    if (window.__fadeAmbience) window.__fadeAmbience(); // restore room ambience when leaving the monitor
  ...
  // Synthesized UI click SFX (no asset): short filtered noise tick + tonal blip.
  // Its own tiny AudioContext, created lazily on first click (a user gesture, so
  // autoplay policy is satisfied). Exposed as window.__click for the desktop UI.
  let sfxCtx = null;
  function playClick(vol = 0.2) {
    try {
      ...
      for (let i = 0; i < len; i++) d[i] = (Math.random() * 2 - 1) * Math.exp(-(i / ctx.sampleRate) / 0.005);
      const src = ctx.createBufferSource(); src.buffer = buf;
      const bp = ctx.createBiquadFilter(); bp.type = "bandpass"; bp.frequency.value = 2700; bp.Q.value = 0.9;
      ...
      og.gain.exponentialRampToValueAtTime(0.0005, now + 0.02);
      o.connect(og); og.connect(ctx.destination); o.start(now); o.stop(now + 0.025);
    } catch (e) { /* SFX is best-effort; never break interaction */ }
  }
  window.__click = playClick;
  renderer.domElement.addEventListener("pointerdown", e => { downXY = [e.clientX, e.clientY]; });
  renderer.domElement.addEventListener("pointerup", e => {
    if (!downXY) return;
    const moved = Math.hypot(e.clientX - downXY[0], e.clientY - downXY[1]);
    downXY = null;
    if (moved <= 6) playClick();                        // tap feedback click
  ...
  backBtn.addEventListener("click", () => { playClick(); zoomHome(); });
  window.addEventListener("keydown", e => { if (e.key === "Escape" && mode === "screen") zoomHome(); });

  // ============================================================
  //  AMBIENCE — office room-tone loop (generated by scripts/gen-ambience.mjs,
  //  original/CC0). Lazy-loaded on first ♪ click, gated behind the user gesture
  //  so it satisfies browser autoplay policy. Seamless loop via AudioBuffer.
  // ============================================================
  // scene.js may be served from a CDN (live) or the dev server (local); resolve
  // the asset relative to THIS script's own URL so both work without a hard-code.
  const SELF = document.currentScript || document.querySelector('script[src*="scene.js"]');
  const ASSET_BASE = SELF && SELF.src ? new URL("../", SELF.src).href : "./";
  const assetUrl = (rel) => new URL(rel, ASSET_BASE).href;

  const AMBIENCE_VOL = 0.35;
  let audio = null, audioOn = false, audioLoading = false;
  // ramp the ambience bus toward a target volume (no-op until audio is loaded)
  function fadeAmbience(vol, ramp = 0.8) {
    if (!audio) return;
    const now = audio.ctx.currentTime;
    audio.gain.gain.cancelScheduledValues(now);
    audio.gain.gain.setValueAtTime(Math.max(0.0001, audio.gain.gain.value), now);
    audio.gain.gain.linearRampToValueAtTime(Math.max(0.0001, vol), now + ramp);
  }
  // target level given user toggle + whether we're zoomed into the monitor
  function ambienceTarget() {
    return audioOn && mode !== "screen" && mode !== "toScreen" ? AMBIENCE_VOL : 0.0001;
  }
  soundBtn.addEventListener("click", async () => {
    ...
    if (!audio) {
      ...
      audioLoading = true;
      try { audio = await initAudio(); }
      catch (e) { console.warn("ambience load failed:", e); audioLoading = false; return; }
      audioLoading = false;
    }
    if (audio.ctx.state === "suspended") await audio.ctx.resume();
    audioOn = !audioOn;
    fadeAmbience(ambienceTarget());
    soundBtn.querySelector("span").textContent = audioOn ? "ON" : "OFF";
  });
  // expose so the zoom transitions can mute/restore the room ambience
  window.__fadeAmbience = () => fadeAmbience(ambienceTarget());
  async function initAudio() {
    ...
    const res = await fetch(assetUrl("assets/audio/ambience.wav") + (isDev ? "?t=" + Date.now() : ""));
